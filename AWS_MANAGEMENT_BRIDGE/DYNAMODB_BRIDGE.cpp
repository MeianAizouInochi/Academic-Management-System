...
namespace AWS_MANAGEMENT_BRIDGE
{
	...
	array<String^>^ DYNAMODB_BRIDGE::GetItems(String^ tablename, String^ partitionkey, String^ partitionvalue)
	{
		...
		std::vector<std::string> Temp_vector = M_Instance->GetItems(table_name, partition_key, partition_value);
